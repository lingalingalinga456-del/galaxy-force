    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="grid gap-3">
        {conversations?.map((c: any) => (
          <Link key={c.id} href={`/talent-dashboard/inbox/${c.id}`}>
            <Card className="p-5 hover:shadow-card-hover transition-all">
              ...
              <div className="text-xs text-warm-muted mt-1">{c.last_message_at ? new Date(c.last_message_at).toLocaleString() : 'No messages yet'}</div>
            </Card>
          </Link>
        ))}
