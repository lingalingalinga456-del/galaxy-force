...
export default async function TalentContractsPage() {
  ...
  const { data: contracts } = await supabase
    .from('contracts')
    .select('id, agreed_amount, currency, status, created_at, client_id, profiles!inner(full_name)')
        ...
        {contracts?.map((c: any) => (
          <Card key={c.id} className="p-6">
            <div className="flex items-start justify-between">
              <div>
                <div className="font-medium text-warm-ink">Client: {c.profiles?.full_name}</div>
